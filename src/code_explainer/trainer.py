...
from .data.datasets import DatasetConfig, build_dataset_dict
...
class CodeExplainerTrainer:
    ...
    def __init__(self, config_path: str = "configs/default.yaml"):
        ...
        setup_logging(
            level=self.config["logging"]["level"], log_file=self.config["logging"]["log_file"]
        )
    ...
    def load_model(self) -> None:
        ...
        logger.info(
            f"Loading model '{model_name}' (arch={self.arch}, dtype={torch_dtype}, 8bit={load_in_8bit})"
        )
    ...
    def load_dataset(self, data_path: Optional[str] = None) -> Any:
        ...
        logger.info(
            f"Loaded {len(ds_dict['train'])} train and {len(ds_dict.get('eval', []))} eval examples"
        )
    ...
    def preprocess_dataset(self, dataset: Any) -> Any:
        ...
        max_tgt_len = int(
            self.config.get("prompt", {}).get("max_explanation_length", max_source_len)
        )
    ...
    def _save_training_metrics(self, output_dir: str) -> None:
        ...
        metrics = {
            "config": self.config,
            "final_metrics": (
                getattr(self.trainer.state, "log_history", [])[-1]
                if getattr(self.trainer, "state", None) and self.trainer.state.log_history
                else {}
            ),
            "device": self.device,
