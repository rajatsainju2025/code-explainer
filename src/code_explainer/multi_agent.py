        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    ...
                    elif any(keyword in node.name.lower() for keyword in ['factorial', 'fibonacci']):
...
"""
    ...
    def _format_conditions(self, symbolic_analysis) -> str:
        ...
        if symbolic_analysis.preconditions:
            conditions.append("- Preconditions: " + ", ".join(
                cond.expression for cond in symbolic_analysis.preconditions[:2]
            ))
        
        if symbolic_analysis.postconditions:
            conditions.append("- Postconditions: " + ", ".join(
                cond.expression for cond in symbolic_analysis.postconditions[:2]
            ))
