...
api-prod: ## Start API server in production mode
	...
	uvicorn code_explainer.api.server:create_app --factory --host 0.0.0.0 --port 8000 --workers 4 --limit-concurrency 100 --backlog 2048 --timeout-keep-alive 20 --no-access-log --log-level info --proxy-headers --forwarded-allow-ips '*'
	@echo "Running with tuned Uvicorn settings (workers=4, keep-alive=20s)"

api-prod-auto: ## Start API server in auto mode
	uvicorn code_explainer.api.server:create_app --factory --host 0.0.0.0 --port 8000 --log-config uvicorn_config.py
	@echo "Running with uvicorn_config.py (auto workers)"
