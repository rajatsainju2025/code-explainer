...
from src.code_explainer.utils import prompt_for_language
...
class TestCodeRetriever:
    ...
    def test_code_retriever_init(self):
        ...
        assert retriever.model_name == "microsoft/codebert-base"
    ...
    def test_retrieve_empty_corpus(self):
        """Test retrieving from empty corpus."""
        retriever = CodeRetriever()
        query = "def test(): pass"
        
        similar_codes = retriever.retrieve_similar_code(query, k=3)
        assert similar_codes == []
    
    def test_retrieve_no_index(self):
        """Test retrieving when no index is built."""
        retriever = CodeRetriever()
        query = "def test(): pass"
...
class TestEnhancedRAGPrompt:
    ...
    def test_enhanced_rag_prompt_no_index(self, tmp_path):
        ...
        config = {
            ...
            "retrieval": {
                "index_path": str(tmp_path / "nonexistent.faiss")
            }
        }
...
class TestRAGIntegration:
    ...
    def test_end_to_end_rag_workflow(self, tmp_path):
        ...
        train_data = [
            {"code": "def fibonacci(n):\n    if n <= 1:\n        return n\n    return fibonacci(n-1) + fibonacci(n-2)", "explanation": "Recursive fibonacci"},
            {"code": "def factorial(n):\n    if n <= 1:\n        return 1\n    return n * factorial(n-1)", "explanation": "Recursive factorial"},
            {"code": "def sum_list(lst):\n    return sum(lst)", "explanation": "Sum of list elements"}
        ]
